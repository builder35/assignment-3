...
argument_parser.add_argument("-ip_addr",metavar ='I', help="ip_address(es) to scan: format can be a single address, comma seperated,CIDR",nargs='?', const ="127.0.0.1")
...
def ping_sweep(hosts,ports):
    ...
    for host in hosts:
        ...
        if "Lost = 0" in output_string: results[host].append(host + " is online")
    return results
